...
class PortfolioStocksEnv(gym.Env):
    ...
    def initial_portfolio_value(self):
        return self._portfolio_value(self._frame_bound[0])

    def current_portfolio_value(self):
        return self._portfolio_value(self._current_tick)
    ...
    def _transfer_allocations(self, source_symbol, target_symbol, current_tick):
        # Temporary Variables
        source_symbol_shares = self._portfolio_allocation[source_symbol]
        source_symbol_price = self._prices_per_symbol[source_symbol][current_tick]

        target_symbol_original_shares = self._portfolio_allocation[target_symbol]
        target_symbol_price = self._prices_per_symbol[target_symbol][current_tick]
    ...
    def _portfolio_value(self, current_tick):
        portfolio_value = 0.0

        for symbol in self._portfolio_allocation:
            symbol_shares = self._portfolio_allocation[symbol]
